...
export default async function ProfilePage({ params }: { params: { username: string } }) {
  ...
  const [weights, recentLogs, inbody] = await Promise.all([
    prisma.weightEntry.findMany({ where: { userId: user.id }, orderBy: { dayNumber: 'asc' } }),
    prisma.dailyLog.findMany({ where: { userId: user.id }, orderBy: { date: 'desc' }, take: 6 }),
    prisma.inBodyEntry.findMany({ where: { userId: user.id }, orderBy: { date: 'asc' } }),
  ])
  ...
  // 目前體脂:使用者手動值優先,留空則帶最新 InBody
  const bodyFatValue = user.bodyFat ?? latestInbody?.bodyFat ?? null
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <section className="mb-8">
        <h1 className="font-serif text-3xl md:text-4xl text-ink">{user.displayName} 的訓練紀錄</h1>
        <p className="text-ink-soft text-sm mt-2">@{user.username}</p>
      </section>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
        ...
        />
        <StatCard label="堅持天數" value={`${persistDays} 天`} sub={`放縱 ${breakDays} 天`} />
        <StatCard
          label="目前體脂"
          value={bodyFatValue != null ? `${bodyFatValue}%` : '—'}
          sub={user.bodyFat != null ? '手動設定' : '最新 InBody'}
        />
      </section>
